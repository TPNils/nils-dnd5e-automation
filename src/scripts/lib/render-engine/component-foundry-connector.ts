...
export class ComponentFoundryConnector {
  ...
  public static registerHooks(): void {
    ...
    UtilsHooks.init().then(() => {
        ...
        for (const mutation of mutationsList) {
          for (const addedNode of Array.from(mutation.addedNodes)) {
            if (addedNode instanceof Element) {
              ComponentFoundryConnector.injectCustomElement(addedNode);
            }
          }
        }
      });
      ...
      document.body.querySelectorAll(`[${attrName}]`).forEach(elem => {
        ComponentFoundryConnector.injectCustomElement(elem);
      });
  ...
  private static injectCustomElement(addedNode: Element): void {
